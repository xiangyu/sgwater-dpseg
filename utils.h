// utility.h
//
// (c) Mark Johnson, 25th August 2004
// modified sgwater 9/5/05 (added more functions)
//
...
#else
#define my_assert(X,Y) if(!(X)){ std::cerr << "data is " << (Y) << std::endl; assert(X); }
#endif

#ifdef NDEBUG
#define debug_output(level, string, value) /* do nothing */
#else
#define debug_output(level, string, value) if (debug_level >= (level)) cout << (string) << (value) << endl;
#endif
...
{ 
  int retval = strtol (str.c_str(), NULL, 10);
#ifndef OS_WINDOWS
  if (retval == 0  &&  errno)
    throw errno;
  else return retval;
#else
  return retval; //Terrible! No error under cygwin!
#endif
}
...
{ 
  long retval = strtol (str.c_str(), NULL, 10);
#ifndef OS_WINDOWS
  if (retval == 0  &&  errno)
    throw errno;
  else return retval;
...
inline void normalize_vector(std::vector<double>& v) {
  double t=0;
...
inline double entropy (std::vector<double>& v) {
  double h=0;
  for (size_t i=0; i<v.size(); i++) {
    if (v[i] > 0)
      h -= v[i]*log(v[i])/log(2);
...
inline double vector_entropy (std::vector<double>& v) {
  return entropy(v);
}


inline std::vector<double>& operator/= (std::vector<double>& v, size_t n) {
  for (size_t i=0; i<v.size(); i++) {
    v[i]/=n;
  }
  return v;
...
inline std::vector<double>& operator/= (std::vector<double>& v, int n) {
  for (size_t i=0; i<v.size(); i++) {
    v[i]/=n;
  }
  return v;
...
inline std::vector<double>& operator/= (std::vector<double>& v, double n) {
  for (size_t i=0; i<v.size(); i++) {
    v[i]/=n;
  }
  return v;
...
inline std::vector<double>& operator+= (std::vector<double>& t, std::vector<double>& v) {
...
inline std::vector<int>& operator+= (std::vector<int>& t, std::vector<int>& v) {
  assert(t.size()==v.size());
  for (size_t i=0; i<v.size(); i++) {
    t[i]+=v[i];
  }
...
inline std::vector<size_t>& operator+= (std::vector<size_t>& t, std::vector<size_t>& v) {
  assert(t.size()==v.size());
  for (size_t i=0; i<v.size(); i++) {
    t[i]+=v[i];
  }
...
struct string_shorterthan {
  bool operator() (const std::string& e1, const std::string& e2) {
...
//
template <class T>
std::ostream& operator<< (std::ostream& os, std::unordered_set<T>& s)
{
  os << '(';
